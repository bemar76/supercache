...
import ch.bemar.supercache.cache.impl.SuperCacheLoader;

public class SuperCacheTestWithoutRemote {
	...
	private static SuperCache configureCache(String other, int port, String name) throws IOException {
		...
		PersistenceLoadChannel<String, String> database = new PersistenceLoadChannel<>();
		database.preload(preload);

		SuperCacheLoader<String, String> loader = new SuperCacheLoader<>(database);

		SuperCache<String, String> cache = new SuperCache<>(name, loader);
